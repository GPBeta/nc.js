...
class Vector {
 public:
  ...
  T& operator[](size_t index) const {
    NCJS_ASSERT(0 <= index && index < length_);
    return start_[index];
  }
...
class StringSearchBase {
 protected:
  ...
  static intptr_t kBadCharShiftTable[kUC16AlphabetSize];
  // Store for the BoyerMoore good suffix shift table.
  static intptr_t kGoodSuffixShiftTable[kBMMaxShift + 1];
  // Table used temporarily while building the BoyerMoore good suffix
  // shift table.
  static intptr_t kSuffixTable[kBMMaxShift + 1];
...
class StringSearch : private StringSearchBase {
 public:
  ...
  static inline int AlphabetSize() {
    ...
    NCJS_ASSERT(sizeof(PatternChar) == sizeof(uint8_t) ||
 ...
 private:
  ...
  static inline intptr_t CharOccurrence(intptr_t* bad_char_occurrence,
  ...
  // pattern.
  intptr_t* bad_char_table() { return kBadCharShiftTable; }

  // Store for the BoyerMoore good suffix shift table.
  intptr_t* good_suffix_shift_table() {
...
size_t StringSearch<PatternChar, SubjectChar>::SingleCharSearch(
    ...
    Vector<const SubjectChar> subject,
    size_t index) {
  NCJS_CHK_EQ(1, search->pattern_.length());
...
size_t StringSearch<PatternChar, SubjectChar>::BoyerMooreSearch(
  ...
  intptr_t* bad_char_occurence = search->bad_char_table();
  intptr_t* good_suffix_shift = search->good_suffix_shift_table();
  ...
  while (index <= subject_length - pattern_length) {
    ...
    while (last_char != (c = subject[index + j])) {
      intptr_t shift = j - CharOccurrence(bad_char_occurence, c);
      index += shift;
    ...
    if (j < start) {
      ...
      intptr_t gs_shift = good_suffix_shift[j + 1];
      intptr_t bc_occ = CharOccurrence(bad_char_occurence, c);
      intptr_t shift = j - bc_occ;
      if (gs_shift > shift) {
        shift = gs_shift;
      }
      index += shift;
...
void StringSearch<PatternChar, SubjectChar>::PopulateBoyerMooreTable() {
  ...
  intptr_t* shift_table = good_suffix_shift_table();
  intptr_t* suffix_table = this->suffix_table();
...
size_t StringSearch<PatternChar, SubjectChar>::BoyerMooreHorspoolSearch(
  ...
  const size_t subject_length = subject.length();
  const size_t pattern_length = pattern.length();
  intptr_t* char_occurrences = search->bad_char_table();
  int64_t badness = -int64_t(pattern_length);

  // How bad we are doing without a good-suffix table.
  PatternChar last_char = pattern[pattern_length - 1];
  intptr_t last_char_shift =
      pattern_length - 1 -
  ...
  while (index <= subject_length - pattern_length) {
    ...
    while (last_char != (subject_char = subject[index + j])) {
      intptr_t bc_occ = CharOccurrence(char_occurrences, subject_char);
      intptr_t shift = j - bc_occ;
      index += shift;
...
void StringSearch<PatternChar, SubjectChar>::PopulateBoyerMooreHorspoolTable() {
  ...
  intptr_t* bad_char_occurrence = bad_char_table();
